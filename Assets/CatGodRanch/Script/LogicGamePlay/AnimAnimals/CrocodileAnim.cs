...
public class CrocodileAnim : AnimTutBase
{
    ...
    {
        firstPost = crocodile.transform.position;
    ...
    private IEnumerator HandleShowTut()
    {
        ...
        yield return crocodile.transform.DOMove(duck.transform.position, 1).WaitForCompletion();
        duck.gameObject.SetActive(false);
        yield return crocodile.transform.DOMove(firstPost, 1).WaitForCompletion();
